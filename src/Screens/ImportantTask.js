...
function ImportantTask() {
  ...
  const sortToEarlier = (event) => {
    ...
    if (first === 10) {
      const sorted = [...ImportantTask].sort(
        (a, b) =>
  ...
  return (
    <Box sx={{ p: { xs: 1, md: 3 } }} className="no-scrollbar">
      ...
      > {ImportantTask?.length >= 1 ? `Important task (${ImportantTask?.length === 1
        ? `only ${ImportantTask?.length} task`
        : `${ImportantTask.length} tasks`})` : "No Task"}
      ...
      >
        {(earlier ? earlier :ImportantTask).map((data, i) => {
          return (
            <Box
            key={i}
            sx={{
