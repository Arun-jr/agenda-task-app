...
function UnCompletedTask() {
  ...
  const UnCompletedTask = task.filter((task) => task.completed !== true);
  ...
  const sortToEarlier = (event) => {
    ...
    if (first === 10) {
      const sorted = [...UnCompletedTask].sort(
        (a, b) =>
  ...
  return (
    <Box sx={{ p: { xs: 1, md: 3 } }} className="no-scrollbar">
      <Typography
        sx={{
          my: 2,
          alignItems: "end",
      ...
      >
        {UnCompletedTask?.length >= 1 ? `Un Completed task (${UnCompletedTask?.length === 1
        ? `only ${UnCompletedTask?.length} task`
        : `${UnCompletedTask.length} tasks`})` : "No Task"}
      
      </Typography>
      <AddTask forAddTask={handleTaskDialog} OpenTask={taskOpen} task={tasks}/>
      ...
      >
        {(earlier ? earlier : UnCompletedTask).map((data, i) => {
          return (
            <Box
              key={i}
              sx={{
              ...
              >
                <Box sx={{ width: 1, height: 1 }}>
                  <Stack direction={"row"} gap={1} mb={2} alignItems={"center"}>
                    ...
                    <IconButton size="small" title="more" onClick={()=> handleEdit(data.id)}>
                      <MoreVert />
                    </IconButton>
                  </Stack>
                 ...
                 {alignment === "grid" ? 
                  ...
                  </Typography> : <Typography sx={{ pb: 2, width: 1 }}>
